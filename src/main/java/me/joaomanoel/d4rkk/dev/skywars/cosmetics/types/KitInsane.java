...
public class KitInsane extends Cosmetic {

  private final String name;
  private final List<Integer> slots;
  private final String icon;
  private final List<KitInsaneLevel> levels;

  public KitInsane(long id, EnumRarity rarity, String permission, String name, List<Integer> slots, String icon, List<KitInsaneLevel> levels) {
    super(id, CosmeticType.INSANEKIT, 0.0D, permission);
    this.name = name;
    this.slots = slots;
    this.icon = icon;
    this.levels = levels;
  ...
  public void apply(Profile profile) {
    ...
    InsaneKitConfig config = profile.getAbstractContainer("aCoreSkyWars", "kitconfig", InsaneKitConfigContainer.class).getOrLoadConfig(this);
    int index = 1;
    for (ItemStack item : this.getCurrentLevel(profile).getItems()) {
      int slot = config.getSlot(index);
      if (!config.isAutoEquipArmor() || !InsaneKitConfig.isArmor(item)) {
        if (slot == -1) {
  ...
  public KitInsaneLevel getFirstLevel() {
    return this.levels.get(0);
  }
  
  public KitInsaneLevel getCurrentLevel(Profile profile) {
    return this.levels.get((int) (profile.getAbstractContainer("aCoreSkyWars", "cosmetics", CosmeticsContainer.class).getLevel(this) - 1));
  }
  
  public List<KitInsaneLevel> getLevels() {
    return this.levels;
  }
