...
import me.joaomanoel.d4rkk.dev.skywars.menus.cosmetics.kitsinsane.level.MenuBuyCashKitLevel;
import me.joaomanoel.d4rkk.dev.skywars.menus.cosmetics.kitsinsane.level.MenuBuyKitLevel;
...
public class MenuKitUpgrade<T extends KitInsane> extends PlayerMenu {
  
  private static final int[] SLOTS = {11, 12, 13, 14, 15, 16, 20, 21, 22, 23, 24, 25};
  private String name;
  private T cosmetic;
  private Map<ItemStack, Integer> intLevels = new HashMap<>();
  private Map<ItemStack, KitInsaneLevel> KitInsaneLevels = new HashMap<>();
  public MenuKitUpgrade(Profile profile, String name, T cosmetic) {
    ...
    int slot = 0;
    for (KitInsaneLevel KitInsaneLevel : cosmetic.getLevels()) {
      int level = slot + 1;
      ...
      String desc = id.equals("13") ?
          ...
          id.equals("4") ?
              Language.cosmetics$kit$icon$buy_desc$start.replace("{rarity}", cosmetic.getRarity().getName()).replace("{cash}", StringUtils.formatNumber(KitInsaneLevel.getCash()))
                  .replace("{coins}", StringUtils.formatNumber(KitInsaneLevel.getCoins())).replace("{buy_desc_status}",
                  ((coins >= KitInsaneLevel.getCoins() || (CashManager.CASH && cash >= KitInsaneLevel.getCash())) ?
                      Language.cosmetics$icon$buy_desc$click_to_buy :
                      Language.cosmetics$icon$buy_desc$enough)) :
              "\n \n&cVocê não pode comprar este upgrade.";
      ItemStack item = BukkitUtils.deserializeItemStack("STAINED_GLASS_PANE:" + id + " : 1 : name>" + color + KitInsaneLevel.getName() + " : desc>" + KitInsaneLevel.getDesc() + desc);
      this.setItem(SLOTS[slot++], item);
      if ((level - 1) == currentLevel) {
        this.intLevels.put(item, level);
        this.KitInsaneLevels.put(item, KitInsaneLevel);
      }
    }
        ...
        if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getInventory())) {
          ...
          if (item != null && item.getType() != Material.AIR) {
            if (evt.getSlot() == 39) {
              ...
              new me.joaomanoel.d4rkk.dev.skywars.menus.cosmetics.kitsinsane.MenuKitConfig<>(profile, this.name, this.cosmetic);
            } else if (evt.getSlot() == 40) {
              EnumSound.CLICK.play(this.player, 0.5F, 2.0F);
              new me.joaomanoel.d4rkk.dev.skywars.menus.cosmetics.kitsinsane.MenuKits<>(profile, this.name, this.cosmetic.getClass());
            } else {
              KitInsaneLevel KitInsaneLevel = this.KitInsaneLevels.get(item);
              if (KitInsaneLevel != null) {
                long level = this.intLevels.get(item);
                if (profile.getCoins("aCoreSkyWars") < KitInsaneLevel.getCoins() && (CashManager.CASH && profile.getStats("aCoreProfile", "cash") < KitInsaneLevel.getCash())) {
                  EnumSound.ENDERMAN_TELEPORT.play(this.player, 0.5F, 1.0F);
                  return;
                }
                
                if (!CashManager.CASH || cosmetic.getCash() == 0) {
                  new MenuBuyKitLevel<>(profile, this.name, cosmetic, KitInsaneLevel, level);
                } else {
                  new MenuBuyCashKitLevel<>(profile, this.name, cosmetic, KitInsaneLevel, level);
                }
              }
            }
          }
  ...
  public void cancel() {
    ...
    this.intLevels = null;
    this.KitInsaneLevels.clear();
    this.KitInsaneLevels = null;
  }
